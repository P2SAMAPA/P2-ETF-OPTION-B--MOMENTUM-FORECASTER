...
if option == "Option A — ARIMA Forecaster":
    ...
    st.caption(
        ...
        "Rank-based composite score · Top 1 ETF · "
        "Fee on switches only · CASH drawdown overlay"
    )
    ...
    # ═════════════════════════════════════════════════════════════════════════
        ...
        with st.spinner("🔮 Computing next-day momentum signal..."):
            ...
            if backtest_ended_in_cash:
                # Reuse cash_days_held from backtest — no recomputation needed.
                # Add 1 for today (the live day after the backtest ended).
                cash_days_held = result.get("cash_days_held", 0) + 1
                in_cash_live   = not should_exit_cash(
                    best_etf, mom_scores, cash_days_held
                )
            else:
                in_cash_live = False
